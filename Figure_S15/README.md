# Figure S15 — Per-poor-prognosis-state (LS) efficacy vs toxicity

**Self-contained.** This folder reproduces deployed **Figure S16** (`../../05_Supplementary_Figures/Figure_S15__ls_headtohead.pdf`) from its own
...
```bash
cd scripts
Rscript figS_ls_headtohead.R
```
The renderer reads `../source_data/*.csv` and writes `figures/`. Deployed PDF is in `05_Supplementary_Figures/` (or `04_Main_Figures/`).

## Contents
- `scripts/` — renderer(s): figS_ls_headtohead.R; builder(s): build_ls_headtohead.py; shared `_style.R` + `config.py` (bundled)
- `source_data/` — plot-level source data (1 file): figS_ls_headtohead.csv
