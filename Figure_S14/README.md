# Figure S14 — Lead-target cell-type specificity

**Self-contained.** This folder reproduces deployed **Figure S14** (`../../05_Supplementary_Figures/Figure_S14__leads_celltype_specificity.pdf`) from its own
...
```bash
cd scripts
Rscript figS_leads_celltype.R
```
The renderer reads `../source_data/*.csv` and writes `figures/`. Deployed PDF is in `05_Supplementary_Figures/` (or `04_Main_Figures/`).

## Contents
- `scripts/` — renderer(s): figS_leads_celltype.R; builder(s): build_leads_celltype.py; shared `_style.R` + `config.py` (bundled)
- `source_data/` — plot-level source data (1 file): figS16_leads_celltype.csv
