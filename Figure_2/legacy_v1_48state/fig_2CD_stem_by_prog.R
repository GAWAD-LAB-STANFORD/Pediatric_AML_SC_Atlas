#!/usr/bin/env Rscript
# Simplified 2C/2D: state-level stemness by prognosis group. Three groups
# (Favorable / No assoc. / Poor); each dot = one leukemic state (per-state median
# score); one panel per score (LSC Eppert, HSC). Kruskal-Wallis + pairwise Wilcoxon.
...
pc  <- read.csv(file.path(SL, "ls48/LS48_stem_percell.csv"))
agg <- aggregate(cbind(LSC_Eppert, HSC) ~ LS, data = pc, median)     # per-state median
pr  <- read.csv(file.path(SL, "target_ls/LS48_prognosis.csv"))
pr$LS <- gsub("_","",pr$LS)
pr$group <- ifelse(pr$fdr < 0.10, ifelse(pr$dir == "favorable", "Favorable", "Poor"), "No assoc.")
...
panel <- function(yvar, ttl, ylab){
  ...
  ggplot(d, aes(group, .data[[yvar]])) +
    ...
    coord_cartesian(ylim = c(rng[1] - pad*0.05, rng[2] + pad*0.85)) }

pL <- panel("LSC_Eppert", NULL, "per-state median LSC score")
pH <- panel("HSC",        NULL, "per-state median HSC score")
fig <- pL + pH + plot_layout(guides = "collect")
ggsave(file.path(OUT, "Figure_2CD_stemness_by_prognosis.pdf"), fig, width = 8, height = 5)
ggsave(file.path(OUT, "Figure_2CD_stemness_by_prognosis.png"), fig, width = 8, height = 5, dpi = 200)
# also print the exact test stats
...
cat("\nwrote Figure_2CD_stemness_by_prognosis\n")
