#!/usr/bin/env Rscript
# Supplemental: annotation of the 48 leukemic states (Jaccard >= 0.35).
# Heatmap of curated hematopoietic lineage-program markers (z per gene across
# states). States labelled by DIFFERENTIATION PROGRAM ONLY (no cytogenetics).
# Right bars: cross-cytogenetic breadth (effective # subtypes a state spans) and
# cross-patient recurrence.
suppressMessages({library(ggplot2); library(patchwork); library(scales)})
source("/private/tmp/claude-503/-Users-chuckgawad-Desktop-ALSF-AML-2026-updated/f5e93d65-31d5-411e-b754-6f7c504928b7/scratchpad/manuscript_palette.R")
SC  <- "/private/tmp/claude-503/-Users-chuckgawad-Desktop-ALSF-AML-2026-updated/f5e93d65-31d5-411e-b754-6f7c504928b7/scratchpad/ls48"
...
PROG <- list("HSC/MPP"=c("CD34","HLF","AVP","CRHBP","HOXA9","MEIS1","MLLT3","PROM1","MECOM","GATA2"),
 ...
 "Cell-cycle"=c("MKI67","TOP2A","CDK1","UBE2C","CENPA","BIRC5","PCNA","NUSAP1","BUB1B"))
g2p <- setNames(rep(names(PROG),lengths(PROG)),unlist(PROG))
z <- read.csv(file.path(SC,"annot_heatmap_z.csv"),row.names=1,check.names=FALSE)
ann <- read.csv(file.path(SC,"LS48_annotation.csv"),check.names=FALSE)
ann$LSn <- as.integer(sub("LS_","",ann$LS)); ann$eff <- ann[["eff_n_cyto"]]
...
ph <- ggplot(long,aes(gene,state,fill=z))+geom_tile()+
  ...
  scale_fill_div(midpoint=0,limits=c(-2.5,2.5),oob=squish,name="z-score")+
...
b1 <- ggplot(ad,aes("cyto\nbreadth",state,fill=eff))+geom_tile(colour="white",linewidth=0.3)+
  scale_fill_seq(name="eff. #\nsubtypes")+
  labs(x=NULL,y=NULL)+theme_minimal(base_size=7)+
...
b2 <- ggplot(ad,aes("patients",state,fill=patient_class))+geom_tile(colour="white",linewidth=0.3)+
  scale_fill_pat(name=NULL)+labs(x=NULL,y=NULL)+theme_minimal(base_size=7)+
  theme(axis.text.y=element_blank(),axis.text.x=element_text(size=6),panel.grid=element_blank(),legend.position="right")
fig <- ph + b1 + b2 + plot_layout(widths=c(1,0.04,0.04))
ggsave(file.path(OUT,"Figure_S_state_annotation.pdf"),fig,width=15,height=10)
ggsave(file.path(OUT,"Figure_S_state_annotation.png"),fig,width=15,height=10,dpi=200)
cat("wrote 48-state annotation heatmap\n")
