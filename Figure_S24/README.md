# Figure S24 — CD96 cell-cell communication (CellChat) + nectin axis

**Self-contained.** This folder reproduces deployed **Figure S24** (`../../05_Supplementary_Figures/Figure_S24__cd96_cellchat.pdf`) from its own
...
## Contents
- `scripts/` — renderer(s): figS8.R; builder(s): build_cd96_axis.py, build_cellchat_3path_input.py; shared `_style.R` + `config.py` (bundled)
- `source_data/` — plot-level source data (6 files): cd96_autocrine_coexpr.csv, cd96_axis_connectivity.csv, cd96_axis_expr.csv, cd96_persample_long.csv, cd96_sample_info.csv, figS22EF_cellchat.csv
