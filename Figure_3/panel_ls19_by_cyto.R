#!/usr/bin/env Rscript
# LS19 deconvolved fraction (%) across TARGET cytogenetic subtypes, coloured by risk group.
# LS19 is zero-inflated (81% zero), so points are jittered and a diamond marks the mean;
# y is sqrt-scaled to show the spread. Shows LS19 enrichment in adverse-risk subtypes.
...
d <- d[!is.na(d$subtype2) & trimws(d$subtype2) != "", c("subtype2","LS_19")]; d$pct <- d$LS_19*100  # drop blank/unclassified cytogenetics
...
ord <- names(sort(tapply(d$pct, d$subtype2, mean)))          # order by mean LS19 %
...
g <- ggplot(d, aes(subtype2, pct, colour=risk)) +
  ...
  labs(x="cytogenetic / molecular subtype", y="LS19 deconvolved fraction (%)") +
  theme_classic(base_size=11) +
  theme(axis.text.x=element_text(angle=40, hjust=1), legend.position="top",
        plot.caption=element_text(size=8, hjust=0))
ggsave(file.path(OUT, "Figure_2_LS19_by_cyto.pdf"), g, width=6.4, height=4.2)
ggsave(file.path(OUT, "Figure_2_LS19_by_cyto.png"), g, width=6.4, height=4.2, dpi=200)
cat("wrote Figure_2_LS19_by_cyto\n")
