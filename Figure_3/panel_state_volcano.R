...
g <- ggplot(d, aes(log2HR, neglogp)) +
  ...
  geom_point(aes(colour = sig, size = state == "LS_10")) +
  geom_point(data = d[d$state=="LS_10",], shape = 21, size = 5, stroke = 1.1, colour = "black", fill = NA) +
  ggrepel::geom_text_repel(data = lab, aes(label = LSc, colour = sig),
      size = 3, fontface = ifelse(lab$state=="LS_10","bold","plain"), max.overlaps = 30, seed = 1, show.legend = FALSE) +
...
ggsave(file.path(OUT, "Figure_3_state_selection_volcano.pdf"), g, width = 6.4, height = 5.2)
ggsave(file.path(OUT, "Figure_3_state_selection_volcano.png"), g, width = 6.4, height = 5.2, dpi = 200)
cat(sprintf("wrote volcano; FDR=0.10 line at -log10 p = %.2f; LS10 at log2HR=%.2f, -log10p=%.1f\n",
    fdr_line, log2(d$WC_coxHR[d$state=="LS_10"]), -log10(d$WC_coxP[d$state=="LS_10"])))
