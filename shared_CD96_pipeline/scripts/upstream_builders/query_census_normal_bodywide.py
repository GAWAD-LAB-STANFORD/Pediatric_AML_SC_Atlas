#!/usr/bin/env python
# ====================================================================
# query_census_normal_bodywide.py  |  CD96 figure pipeline component
# reproducibility header (auto-generated; edit freely below this block)
# Inputs : (reads upstream object / raw matrix — see body)
# Outputs: (writes to ../figures or ../source_data — see body)
# Run    : python query_census_normal_bodywide.py   (or: bash run_all.sh)  |  Deps: see REPRODUCIBILITY_CHECKLIST.md
...
OUT_TISSUE = os.path.join(HERE, "Figure8_census_normal_by_tissue.csv")
OUT_CT = os.path.join(HERE, "Figure8_census_normal_by_celltype.csv")
OUT_CT_GLOBAL = os.path.join(HERE, "Figure8_census_normal_celltype_global.csv")
...
def main():
    ...
    with cellxgene_census.open_soma(census_version=CENSUS_VERSION) as census:
        ...
        for ti, tis in enumerate(tissues, 1):
            ...
            il_t = ""
            if "IL3RA" in genes_present:
                j = genes_present.index("IL3RA")
            ...
            for cname, gd in glob.items():
                if "plasmacytoid" in cname.lower() and gd["tot"]:
                    p = 100.0 * gd["pos"][gi_of["IL3RA"]] / gd["tot"]
                    pdc_str = f"  [pDC running: IL3RA={p:.2f}% of {gd['tot']:,}]"
            print(f"  [{ti}/{len(tissues)}] {tis:22s} kept={n_kept:>9,}  "
                  f"IL3RA={il_t}%  ct_rows={n_ct_written}{pdc_str}", flush=True)
